...
const getRequestByTenantId = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const tenantId = req.user?.id;
    const result = await requestService.getRequestByTenantIdDB(tenantId as string);
    sendResponse(res, {
        success: true,
